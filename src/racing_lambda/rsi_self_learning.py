"""競馬予想システム開発・本格先行予測λ専用のRSI自己学習部品。"""
...
RACING_RSI_PERIODS = (5, 7, 14, 21)
RACING_RSI_FEATURE_VERSION = "racing-rsi-self-learning-v1"


def calculate_support_rsi(values: pd.Series, period: int) -> pd.Series:
    """Calculate Wilder RSI from chronological implied-support observations."""
    if period < 2:
...
def latest_rsi_state(values: Iterable[float], period: int) -> dict[str, float]:
    """Return outcome-neutral RSI state; 30/70 never directly means buy/sell."""
    series = pd.Series(list(values), dtype=float).dropna().reset_index(drop=True)
    rsi = calculate_support_rsi(series, period).dropna()
    if rsi.empty:
        return {
            "level": 0.0,
            "velocity3": 0.0,
    ...
    previous = float(rsi.iloc[-2]) if len(rsi) >= 2 else latest
    velocity = (latest - float(rsi.iloc[-4])) / 100.0 if len(rsi) >= 4 else 0.0
...
class RsiLearningSummary:
    rows: int
    positive_rows: int
    feature_count: int


class RacingRsiOutcomeLearner:
    """Ridge learner that links past PRE_RACE RSI states to later results."""
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "RacingRsiOutcomeLearner":
        rsi_columns = [column for column in features if column.startswith("rsi_")]
        aligned = features.loc[:, rsi_columns].replace([np.inf, -np.inf], np.nan)
        ...
        if len(aligned) < 6 or target.nunique() < 2:
            raise ValueError("RSI result learning needs at least six labeled rows and two classes")
        variable = aligned.std(axis=0, ddof=0) > 1e-12
        aligned = aligned.loc[:, variable]
        if aligned.shape[1] < 1:
            raise ValueError("RSI result learning needs at least one variable RSI feature")
        ...
        self.summary_ = RsiLearningSummary(
            rows=len(target),
