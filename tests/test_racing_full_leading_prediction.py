...
def test_public_jra_pre_race_snapshots_feed_full_model_features():
    ...
    assert "wsi_win_5_level" in frame.columns
...
def test_wsi_self_learning_uses_only_prior_results_and_scores_target_race():
    ...
    assert model.wsi_feature_version == RACING_WSI_FEATURE_VERSION
    assert model.wsi_learning_summary_.rows == 16
    scored = model.score_jra_race_research(dense_race_history("TARGET", 1), scheduled_start=RACE_START)
    assert all(row.wsi_self_learning_score is not None for row in scored)
    assert all(0.0 <= row.combined_score <= 1.0 for row in scored)
    assert all(row.wsi_feature_count > 0 for row in scored)


def test_wsi_does_not_change_past_values_when_future_support_arrives():
    support = pd.Series([0.10 + 0.002 * index for index in range(30)])
    original = calculate_support_wsi(support, 14)
    extended = calculate_support_wsi(pd.concat([support, pd.Series([0.01])], ignore_index=True), 14)
...
def test_full_lambda_adaptive_wsi_keeps_pca_regularization_and_blocks_old_target():
    from datetime import timedelta
    from racing_lambda.adaptive_wsi_bridge import WsiBridgeObservation
    ...
    observations = [WsiBridgeObservation(
        mode="full", race_id=f"H{day}", horse_id=str(horse), field_size=5,
        wsi_trained_until=base + timedelta(days=day, hours=-3),
        ...
        wsi_score=0.90 if horse <= 3 else 0.10,
        top3=horse <= 3,
    ) for day in range(8) for horse in range(1, 6)]
    model.fit_wsi_bridge(observations,
                         prediction_at=datetime(2026, 9, 9, 5, tzinfo=timezone.utc))
    assert model.adaptive_wsi_bridge.weight_ > 0
    ...
    assert all(row.adaptive_wsi_weight == model.adaptive_wsi_bridge.weight_ for row in scored)
